...
Response Rules:
1. ONLY generate HTML that is correct. You MUST reply with ONLY the INNER HTML of the existing div that is the artboard, nothing else.
2. Your response must ONLY include the raw HTML without any markdown code blocks, explanations or comments.
3. Images must be described in detail, this description will be the prompt to generate an image, be specific.
4. ALL styling MUST be defined in the class attribute of the element, using tailwind css and tailwindcss ONLY. There MUST NOT be ANY other CSS or custom styles.
5. Your content will be rendered inside the frame.
...
current artboard size: ${artboard_size}.
You can change the artboard size in the meta tag i.e. <meta data-width="600" data-height="800">
This is NOT an interactive desin it is for a static image.

Included is the current design as an image and the following is the current design in HTML format.
MUST edit this HTML to create the best design possible, if you do not include an item that is in the current design HTML it will be deleted from the canvas, this is okay but only do it if it is necessary.
...
`
	...
	console.log('@CHECKPOINT desy', chat_history_messages.length);
	for (let i = 0; i < chat_history_messages.length; i++) {
		console.log(
			'@CHECKPOINT 1',
			JSON.stringify(chat_history_messages[i].content).substring(0, 100)
